...
class CheckoutServletBankTransferPendingTest {
    ...
    void bankTransferCheckoutShouldBePendingVerification() throws Exception {
        ...
        when(mockProductDAO.reserveStock(eq(mockConn), eq(12), eq(1))).thenReturn(true);
        ...
        when(mockInventoryBatchDAO.hasTrackedBatchesForProduct(any(), eq(12))).thenReturn(false);
    ...
    @Test
    @DisplayName("checkout should persist the same shipping fee shown on the checkout page")
    void checkoutShouldUseRenderedShippingFeeWhenSavingOrder() throws Exception {
        CartDAO mockCartDAO = mock(CartDAO.class);
        UserDAO mockUserDAO = mock(UserDAO.class);
        AddressDao mockAddressDAO = mock(AddressDao.class);
        ProductDAO mockProductDAO = mock(ProductDAO.class);
        ...
        Product cartProduct = new Product();
        cartProduct.setId(12);
        cartProduct.setName("Hat meo");
        cartProduct.setPrice(new BigDecimal("100000"));
        cartProduct.setWeight(300);
        cartProduct.setStock(8);
        ...
        Product lockedProduct = new Product();
        lockedProduct.setId(12);
        lockedProduct.setName("Hat meo");
        lockedProduct.setPrice(new BigDecimal("100000"));
        lockedProduct.setWeight(300);
        lockedProduct.setStock(8);
        ...
        try (MockedStatic<DBContext> mockedDBContext = mockStatic(DBContext.class);
             MockedConstruction<ShippingService> mockedShipping = mockConstruction(
                     ShippingService.class,
                     (mock, context) -> when(mock.calculateShippingFee(anyString(), anyString(), anyString(),
                             anyInt(), anyInt(), anyInt(), anyInt())).thenReturn(42000))) {
            mockedDBContext.when(DBContext::getConnection).thenReturn(mockConn);
            servlet.doPost(request, response);
        }

        verify(mockOrderDAO).saveOrder(eq(mockConn), argThat(order ->
                new BigDecimal("142000").compareTo(order.getTotalAmount()) == 0));
    }
