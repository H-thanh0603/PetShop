...
public class ProductDAO {
 ...
 // 8. Lấy 4 sản phẩm liên quan (trừ sản phẩm đang xem)
    ...
    // Pagination methods for shop page
    public List<Product> getPopularProductsPage(int page, int pageSize) {
        List<Product> list = new ArrayList<>();
    ...
    public List<Product> getDiscountedProductsPage(int page, int pageSize) {
        List<Product> list = new ArrayList<>();
        int offset = (page - 1) * pageSize;
        String query = "SELECT * FROM products WHERE discount > 0 ORDER BY discount DESC LIMIT ? OFFSET ?";
        try (Connection conn = DBContext.getConnection();
             PreparedStatement ps = conn.prepareStatement(query)) {
            ps.setInt(1, pageSize);
            ps.setInt(2, offset);
